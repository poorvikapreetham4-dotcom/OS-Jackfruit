...
Supervisor daemon — started once with engine supervisor ./rootfs-base. It stays alive, manages containers, and owns the logging pipeline.
CLI client — each command like engine start alpha ./rootfs-alpha /bin/sh is a short-lived process that sends a request to the running supervisor and prints the response.
...
No log lines are dropped when a container exits abruptly
The buffer does not deadlock when it is full and a container is trying to log
Consumer threads observe a termination signal and flush all remaining entries before exiting
...
In your README.md, include an analysis section that connects your implementation to OS fundamentals. This is not a description of what you coded - it is an explanation of why the OS works this way and how your project exercises those mechanisms.
...
monitor_ioctl.h — shared ioctl definitions between user and kernel space
At least two workload/test programs used for memory and scheduling demonstrations
Makefile — must support building all of the above with a single make
README.md
...
Address the five areas described in the Engineering Analysis section above. This is not a description of what you coded — it is an explanation of why the OS works this way and how your project exercises those mechanisms.
